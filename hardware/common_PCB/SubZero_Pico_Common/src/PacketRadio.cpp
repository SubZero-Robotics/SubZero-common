...
PacketRadio::PacketRadio(SPIClass* spi, Configuration config, std::function<void(Message)> newDataCb) : _cb(newDataCb), _config(config) {
    _radio = std::make_unique<RFM69_ATC>(Radio::RadioCS, Radio::RadioInterrupt, true, spi);
}

void PacketRadio::init() {
    _radio->initialize(Radio::Frequency, hashTeamNumber(_config.teamNumber), Radio::NetworkId);

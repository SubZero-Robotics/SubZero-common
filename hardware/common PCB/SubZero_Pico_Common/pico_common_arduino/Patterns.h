...
static bool executePatternChase(Adafruit_NeoPixel &strip, uint32_t color,
                                uint16_t state, uint16_t ledCount) {
  if (state - 1 >= 0) {
...
static bool executePatternWipe(Adafruit_NeoPixel &strip, uint32_t color,
                               uint16_t state, uint16_t ledCount) {
...
static Pattern patterns[patternCount] = {
    ...
    {.type = PatternType::Wipe,
     .numStates = ledNum,
     .changeDelayDefault = 500u,
     .cb = Animation::executePatternWipe}};
} // namespace Animation

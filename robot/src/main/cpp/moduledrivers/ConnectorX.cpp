...
void ConnectorX::configureDigitalPin(DigitalPort port, PinMode mode) {
    ...
    cmd.commandData.commandDigitalSetup = {
        .port = (uint8_t)port,
        .mode = (uint8_t)mode
    };
...
void ConnectorX::writeDigitalPin(DigitalPort port, bool value) {
    ...
    cmd.commandData.commandDigitalWrite = {
        .port = (uint8_t)port,
        .value = (uint8_t)value
    };
...
bool ConnectorX::readDigitalPin(DigitalPort port) {
    ...
    cmd.commandData.commandDigitalRead = {
        .port = (uint8_t)port
    };
...
uint16_t ConnectorX::readAnalogPin(AnalogPort port) {
    ...
    cmd.commandData.commandReadAnalog = {
        .port = (uint8_t)port
    };
...
void ConnectorX::setPattern(LedPort port, PatternType pattern, bool oneShot = false,
                  int16_t delay = -1) {
    setLedPort(port);
...
void ConnectorX::setColor(LedPort port, uint8_t red, uint8_t green, uint8_t blue) {
    setLedPort(port);
...
Commands::Response ConnectorX::sendCommand(Commands::Command command, bool expectResponse) {
    ...
    // TODO: If response expected, receiveSize = responseData size based on responseType
}